...
function isIgnored(filePath: string, vaultPath: string, ignoreList: string[]): boolean {
	...
	return ignoreList.some(
		(pattern) => rel === pattern || rel.startsWith(pattern + '/') || rel.startsWith(pattern)
	);
}
...
function scanVault(config: SyncConfig): DiscoveredNote[] {
	...
	for (const filePath of allMdFiles) {
		...
		for (const mapping of sortedMappings) {
			const matchingTag = fm.tags.find((t) => t === mapping.tag || t.startsWith(mapping.tag + '/'));
			if (matchingTag) {
				if (!fm.slug) {
...
/** Parse the cover field from Obsidian frontmatter: [[filename.jpg]] → filename.jpg */
function parseCoverFilename(cover: unknown): string | null {
...
function buildBlogFrontmatter(
	...
	if (ogImagePath) {
		lines.push(`ogImage: ${yamlString(ogImagePath)}`);
	}
	lines.push('---');
	return lines.join('\n');
}
...
/** Optimize a cover image for OG (max 1200px wide, preserve aspect ratio) */
async function optimizeOgImage(sourcePath: string, outputPath: string): Promise<void> {
	mkdirSync(dirname(outputPath), { recursive: true });
	await sharp(sourcePath)
...
async function runSync() {
	...
	for (const note of notes) {
		...
		// Process OG cover image
		let ogImagePath: string | undefined;
		const coverFilename = parseCoverFilename(note.frontmatter.cover);
		if (coverFilename) {
			const coverSource = findImageInVault(config.vaultPath, coverFilename);
			if (coverSource) {
				const ogRelPath = join('static/media/posts', slug, 'og-cover.webp');
				const ogAbsPath = join(ROOT, ogRelPath);

				const srcHash = fileHash(coverSource);
				const cachedHash = imageCache[ogRelPath];

				if (cachedHash === srcHash && existsSync(ogAbsPath)) {
					newImageCache[ogRelPath] = srcHash;
				} else {
					await optimizeOgImage(coverSource, ogAbsPath);
					newImageCache[ogRelPath] = srcHash;
					summary.imagesAdded.push(ogRelPath);
				}

				newLockFiles.push(ogRelPath);
				ogImagePath = `/media/posts/${slug}/og-cover.webp`;
			} else {
				console.warn(`⚠ Cover image not found in vault: ${coverFilename}`);
			}
		}

		// Build output
		const frontmatter = buildBlogFrontmatter(note.frontmatter, title, ogImagePath);
